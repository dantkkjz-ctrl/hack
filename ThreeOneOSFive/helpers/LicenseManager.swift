...
final class LicenseManager: ObservableObject {
    ...
    init() {
        // If a key was previously saved, activate locally without any network call
    ...
    func activate(key: String, isAutoLogin: Bool = false) {
        ...
        // Local activation — only key "1" is accepted
        guard trimmedKey == "1" else {
            isActive = false
        ...
        if rememberKey {
            save(trimmedKey, for: keyAccount)
        }
    }
